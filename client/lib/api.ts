...
// Projects
export const createProject = async (body: { clientName: string; description: string }): Promise<Project> => {
...
// Analytics
export const getHumanConfidence = async (projectId: string): Promise<{ confidence: number }> => {
  const { data } = await api.get(`/api/analytics/project/${projectId}/human-confidence`);
  return { confidence: data.confidence }; // Extract from { message, confidence }
};

export const downloadClicks = async (projectId: string, format: 'json' | 'csv'): Promise<Blob> => {
  const { data } = await api.get(`/api/analytics/project/${projectId}/export?format=${format}`, {
    responseType: 'blob',
  });
  return data;
};
...
export const getClicksPerPlatform = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/clicks-per-platform`);
...
export const getGlobalReach = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/global-reach`);
  return { data: data.data || [] }; // Extract from { message, data }
};
...
export const getPlatformEfficiency = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/efficiency`);
  return { data: data.data || [] }; // Extract from { message, data }
};
...
export const getDeviceBreakdown = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/device-breakdown`);
  return {
...
export const getRecentActivity = async (projectId: string): Promise<{ data: Click[] }> => {
  const { data } = await api.get(`/api/analytics/project/${projectId}/recent-activity`);
  return { data: data.data || [] }; // Extract from { message, data }
};
...
export const getPlatformDominance = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/platform-dominance`);
  return { data: data.data || [] }; // Extract from { message, data }
};
...
export const getNetworkEffect = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/network-effect`);
  return {
    totalClicks: data.totalClicks || 0,
...
export const getVelocity = async (projectId: string): Promise<{ growth: number }> => {
...
export const getHeroCampaign = async (
  ...
  const { data } = await api.get(`/api/analytics/project/${projectId}/hero-campaign`);
  return {
...
export const getMonthlyHealth = async (projectId: string): Promise<AnalyticsData> => {
  const { data } = await api.get(`/api/analytics/project/${projectId}/monthly-health`);
  return {
