    ...
    try {
        if (!project_id || !campaign_id || !title) {
            return res
                .status(400)
                .json({
                    message: "Invalid credentials: missing project_id, campaign_id, or title",
                });
        }
...
export const fetchCampaignDetails = async (req, res) => {
    ...
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(campaign_id)) {
            return res
                .status(400)
                .json({ message: "Invalid credentials: invalid campaign_id format" });
        }
...
export const deleteCampaign = async (req, res) => {
    try {
        ...
        if (confirmation !== true) {
            return res
                .status(400)
                .json({ message: "Invalid credentials: deletion must be confirmed" });
        }
...
export const fetchCampaignLinks = async (req, res) => {
    ...
    try {
        ...
        if (!links) {
            return res
                .status(404)
                .json({ message: "We found no associated links for this campaign" });
        } else {
            return res.status(200).json({
